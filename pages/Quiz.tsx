...
const Quiz: React.FC = () => {
  const { courseId, sectionIndex, itemIndex } = useParams();
    ...
    const fetchCourse = async () => {
      ...
      try {
        ...
        const mappedCourse = {
          ...
          content: fetchedCourse.content || [], // Use content, not syllabus
  ...
  // Extract quiz data safely - do this BEFORE any hooks that depend on it
  const quizItem = course?.content?.[parseInt(sectionIndex || '0')]?.items?.[parseInt(itemIndex || '0')];
  const itemId = `${sectionIndex}-${itemIndex}`;
  const questions = quizItem?.type === 'quiz' ? (quizItem.questions || []) : [];
  const timeLimit = quizItem?.type === 'quiz' ? quizItem.timeLimit : undefined;
  
  // Initialize quiz state - THIS MUST BE CALLED UNCONDITIONALLY
  ...
  // Initialize state only once when questions are loaded
  useEffect(() => {
    if (questions.length > 0 && !initialized) {
      setSelectedAnswers(initialData.answers);
      setQuestionStatuses(initialData.statuses);
      setInitialized(true);
    }
  }, [questions.length, initialized, initialData]);

  // Handler functions for quiz
    const handleQuizCompletion = async (courseIdNum: number, itemIdStr: string, score: number) => {
    if (!user) return;
    
    try {
      
      // Mark quiz as complete via backend API
      ...
      console.error('❌ Error updating quiz completion:', err);
    }
  };

  const handleSaveQuizProgress = async (courseIdNum: number, itemIdStr: string, answers: number[], statuses: QuestionStatus[]) => {
    if (!user) return;
  ...
  // Calculate score and stats - these hooks must be called unconditionally
  const score = useMemo(() => {
    if (questions.length === 0 || selectedAnswers.length === 0) return 0;
  ...
  const finalScoreOutOf10 = useMemo(() => {
    ...
    return Math.round((score / questions.length) * 10);
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-screen"><div>Loading quiz...</div></div>;
  }

  if (!course || !quizItem || quizItem.type !== 'quiz' || !quizItem.questions || questions.length === 0) {
  ...
  const handleSubmit = async () => {
    ...
    // Calculate score directly here (score state might not be updated yet)
    ...
    if (user && course) {
      try {
        ...
        // Call the backend API to save quiz score
        ...
        // Refresh user data to get updated enrollments, quiz scores and certificates
        await refreshUser();
      } catch (err) {
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-lg border border-border">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-heading mb-4">Quiz Results</h1>
          <div className="inline-block bg-primary/10 rounded-lg p-6 border-2 border-primary">
            <p className="text-sm text-content mb-2">Your Score</p>
            <p className="text-5xl font-bold text-primary">{finalScoreOutOf10}<span className="text-2xl text-content">/10</span></p>
            <p className="text-sm text-content mt-2">({score} out of {questions.length} questions correct)</p>
          </div>
        </div>
        ...
        <h2 className="text-xl font-bold text-heading mb-4">Review Your Answers</h2>
        ...
        <div className="flex gap-4 mt-8">
          <Link to={`/courses/${courseId}`} className="flex-1 text-center bg-primary hover:bg-primary-dark text-white font-bold py-3 px-6 rounded-lg transition-colors">
            Continue Learning
          </Link>
          <button 
            onClick={() => window.location.reload()} 
            className="flex-1 bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold py-3 px-6 rounded-lg transition-colors"
          >
            Retake Quiz
          </button>
        </div>
  ...
  return (
    <div className="max-w-full mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-heading">{quizItem.title}</h1>
